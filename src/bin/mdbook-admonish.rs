...
const ADMONISH_JS: &[u8] = include_bytes!("assets/mdbook-admonish.css");
const ADMONISH_FILES: &[(&str, &[u8])] = &[("mdbook-admonish.css", ADMONISH_JS)];

pub fn make_app() -> Command<'static> {
    Command::new("mdbook-admonish")
        .version(crate_version!())
        .about("mdbook preprocessor to add admonish support")
        .subcommand(
